...
def process_hand(cards, played_cards, tricks_won, tricks_predicted):
    # Process cards in hand (4 cards)
    encoded_hand = [one_hot_encode_card(card) for card in cards]

    # Process played cards by other players
    # If no information is available, return a null vector (all zeros)
    encoded_played_cards = []
    for card in played_cards:
        if card is None:
            encoded_played_cards.append(torch.zeros(17))  # Null vector
        else:
            encoded_played_cards.append(one_hot_encode_card(card))

    # Combine the hand and played cards
    combined_cards = encoded_hand + encoded_played_cards
...
played_cards = [
    None,  # No information available for player 1
    Card(value='9', suit='Spades'),  # Card played by player 2
    Card(value='Jack', suit='Hearts')  # Card played by player 3
]
...
hand_tensor = process_hand(hand, played_cards, tricks_won, tricks_predicted)
